...
```{r setup, include="hide", include=FALSE, warning=FALSE, message=FALSE}
...
head(income)
```
Because in the dataset provided by the company, the states are represented by the abbreviation and not by the full name, another table was extracted from the following website. The table included also the Commonwealth territories which were dropped from the dataset.
...
DF2%>%
  group_by(GENDER)%>%
  summarise(no_rows=length(GENDER=='F'))
```
...
To see the trend of customers' number throughout the year, they were grouped by SNAPSHOT_DATE.
...
```
Furethermore, we want se see the correlation between the change in paying rate and the number of calls.
```{r}
V1$CALL_COUNT <- as.numeric(V1$CALL_COUNT)

ggplot(V2, aes(x = PAYING_RATE_CHANGE, y= CALL_COUNT)) + geom_point(color="red", size = 2, shape = 23) + xlab("Change in paying rate") + ylab("Calls")
```

```{r message=FALSE, include=FALSE}
DF3$CREDIT_CLASS <- as.numeric(DF3$CREDIT_CLASS)
...
Before building a model, it is necessary to see how many instances of CHURN there are in the dataset.
```{r}
table(DF3$CHURN)
```
...
The logistic regression model was created based on training balanced data.
```{r}
...
accuracy.meas(test.data$CHURN, pred_test, threshold = 0.5)
```